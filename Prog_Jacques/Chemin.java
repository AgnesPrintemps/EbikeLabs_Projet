...
class Chemin{
	...
	// constructeur
	public Chemin(File nom, Vent v, Cycliste c, double temp){
		...
		for(int i=1; i<d.getTailleEchantillon()-1;i++){ // puis, pour chaque point
			...
			Segment s=new Segment(new Point(d.getLongitude(i-1), d.getLatitude(i-1)),new Point(d.getLongitude(i), d.getLatitude(i)), v, c, temp); // puis, on crée un segment entre ces points
